...
from database.async_sync import (
    _sync_session_async,
    make_sync_callback,
    queue_sync_task,
)
...
class TestMakeSyncCallback:
    ...
    def test_callback_accepts_path(self) -> None:
        ...
        callback = make_sync_callback("/path/to/project")
        # Should not raise
        with patch("database.async_sync.queue_sync_task"):
            callback(Path("/path/to/session"))

    @patch("database.async_sync.queue_sync_task")
...
class TestQueueSyncTask:
    ...
    def test_derives_working_dir_from_session_dir(self) -> None:
        ...
        with patch("database.async_sync.asyncio.get_running_loop") as mock_loop:
    ...
    @patch("database.async_sync.asyncio.get_running_loop")
...
class TestSyncSessionAsync:
    ...
    @pytest.mark.asyncio
    @patch("database.async_sync.get_async_session")
    @patch("database.async_sync.sync_session_from_filesystem")
