 ...
 *
 * Serves state.json content for a session from the filesystem.
 * Looks up session by slug, constructs session directory path, reads state.json.
 */
  ...
  try {
    ...
    try {
      ...
      if ((fileError as NodeJS.ErrnoException).code === "ENOENT") {
        return NextResponse.json(
          { error: `state.json not found for session '${slug}'` },
          { status: 404 }
        );
      }
      // JSON parse error
      if (fileError instanceof SyntaxError) {
        return NextResponse.json(
          { error: `Invalid JSON in state.json: ${fileError.message}` },
          { status: 500 }
        );
      }
      throw fileError;
